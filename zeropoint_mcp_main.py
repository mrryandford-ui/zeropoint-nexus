...
def safe_join(root: str, path: str) -> str:
    ...
    if not joined.startswith(os.path.normpath(root) + os.sep) and os.path.normpath(
        joined
...
TOOL_REGISTRY = {
    ...
    "webtools_scrape": {
        "description": "Fetch and scrape HTML via CSS selector",
        "schema": {"url": "http://...", "selector": "CSS selector (optional)"},
    },
    "adb_devices": {"description": "List adb devices using system adb", "schema": {}},
    "adb_screenshot": {
        "description": "Capture device screenshot via adb exec-out screencap -p",
        "schema": {"serial": "(optional) device serial"},
    },
}
